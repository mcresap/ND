//Maria Cresap and Adam Chvilicek final project
//project.cpp
//brick breaker game played using the right and left arrow keys to move the paddle and space bar to start the ball and left click to play or replay
#include "gfxnew.h"
#include <vector>
#include <unistd.h>
#include <cmath>
...
int main()
{
    ...
    int dx=rand() % 16 - 8;
    int dy=rand() % 9;
    if (abs(dx)<3 || abs(dy)<3){ //randomize starting velocity of ball
        dx=4;
        dy=4;
    }
    ...
    while (c!='q'){
        ...
        while (lives>0){
            ...
            if (c=='Q'){ //left arrow key to move paddle and ball to start
                if (padx>0){
                    gfx_color(0, 0, 0);
                    gfx_circle(ballx, bally, r);
                    gfx_fill_rectangle(padx, pady, padw, padh);
            ...
            if (c=='S'){ //right arrow key to move paddle and ball to start
                if (padx+padw<w){
                    gfx_color(0, 0, 0);
                    gfx_fill_rectangle(padx, pady, padw, padh);
                    gfx_circle(ballx, bally, r);
            ...
            while (began==true){
                ...
                // allow user to move paddle to left and right while playing
